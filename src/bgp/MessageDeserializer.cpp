...
template<bool StopOnFirstError>
route::bgp::DeserializeMessageResult
route::bgp::MessageDeserializer<StopOnFirstError>::deserialize(uint8_t *buffer, const uint16_t buf_size) {
    ...
    if (buf_sync_err) {
        build_err(result.errors, BGP_ERR_MESSAGE_HEADER, BGP_SUBERR_MESSAGE_HEADER_CONN_NOT_SYNC, std::nullopt);
        return result;
    }

    // Validate header
    Header *header = Header::from_buffer(buffer);
    if (bad_msg_size(header->length) ||
        ...
        build_err(
                ...
                [length = header->length](DeserializeMessageErrorResult &error) -> uint16_t {
...
template<bool StopOnFirstError>
void route::bgp::MessageDeserializer<StopOnFirstError>::deserialize_open(uint8_t *body_buffer,
    ...
    if (partial_msg->version != BGP_VERSION) {
        build_err(
                ...
                [](DeserializeMessageErrorResult &error) -> uint16_t {
                    error.message.data = std::make_unique<uint8_t[]>(2);
...
template<bool StopOnFirstError>
void route::bgp::MessageDeserializer<StopOnFirstError>::deserialize_update(
        uint8_t *body_buffer,
        uint16_t body_buf_size,
...
template<bool StopOnFirstError>
void route::bgp::MessageDeserializer<StopOnFirstError>::deserialize_notification(
        uint8_t *body_buffer,
        uint16_t body_buf_size,
        route::bgp::DeserializeMessageResult &result) {
    //TODO
...
template<bool StopOnFirstError>
void route::bgp::MessageDeserializer<StopOnFirstError>::build_err(std::vector<DeserializeMessageErrorResult> &error,
                                                                  uint8_t code,
                                                                  uint8_t subcode,
    ...
    error.emplace_back();
    DeserializeMessageErrorResult &latest_error = error.back();
    latest_error.message.error_code = code;
    latest_error.message.error_subcode = subcode;

    uint8_t data_size = 0;
    if (build_err_data.has_value()) {
        data_size = build_err_data.value()(latest_error);
    }

    latest_error.header.length = BGP_MSGSIZE_NOTIFICATIONMSG_MIN + data_size;
}
